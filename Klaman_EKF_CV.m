...
Xekf(:,1)=Tar; % initial value for filter,column vector
Phi=[eye(3),delta_T*(eye(3));zeros(3),eye(3)];
Gamma=[eye(3);zeros(3)];
Q=[5e-3*eye(3) zeros(3);zeros(3) 1e-3*eye(3)]; % km system noise matrix
R=1/57.3*[0.02 0 0 0; 0 0.02 0 0; 0 0 0.02 0; 0 0 0 0.02]; % degree observe noise
P0=eye(6);% covariance matrix
time(1)=0;
...
% Jacobi matrix
H=[-y1/(x1^2+y1^2) x1/(x1^2+y1^2)  0 0 0 0;
    -x1*z1/(sqrt(x1^2+y1^2)*(x1^2+y1^2+z1^2)) -y1*z1/(sqrt(x1^2+y1^2)*(x1^2+y1^2+z1^2)) sqrt(x1^2+y1^2)/(x1^2+y1^2+z1^2) 0 0 0;
   -y2/(x2^2+y2^2) x2/(x2^2+y2^2)  0 0 0 0;
    -x2*z2/(sqrt(x2^2+y2^2)*(x2^2+y2^2+z2^2)) -y2*z2/(sqrt(x2^2+y2^2)*(x2^2+y2^2+z2^2)) sqrt(x2^2+y2^2)/(x2^2+y2^2+z2^2) 0 0 0];
